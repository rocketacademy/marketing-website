...
const StartCodingSection = ({ upcomingCourseDates }) => {
  ...
  if (upcomingCourseDates) {
    const basicsDates = upcomingCourseDates.filter((course) =>
      course.node.summary.includes("Basics")
    );
    nextBasicsStartDate = basicsDates[0].node.start.dateTime;

    const fulltimeDates = upcomingCourseDates.filter((course) =>
      course.node.description.includes("Full Time")
    );
    nextFulltimeStartDate = fulltimeDates[0].node.start.dateTime;

    const parttimeDates = upcomingCourseDates.filter((course) =>
