...
const BootcampCourseSection = ({ course }) => {
  return (
    <div
      className="container bootcamp-course-container section"
      id={course.sidebarlabel.replace(/ /g, "-").toLowerCase()}
    >
      <div className="bootcamp-course-header">{course.heading}</div>
      <MarkdownContent
        content={course.subheading}
        className={"bootcamp-course-subheading"}
      />
          ...
          <tbody>
            {course.schedule.map((section) => {
              return (
                <tr>
                  <td>
                    <p className="bootcamp-course-module">{section.heading}</p>
                  </td>
